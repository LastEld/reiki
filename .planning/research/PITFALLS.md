...
**Why it happens:**
Developers use NEXT*PUBLIC* prefix incorrectly or reference server-only variables in client components.
...
- Only use NEXT*PUBLIC* for truly public values (analytics IDs, publishable keys)
...
| Stripe                  | Exposing secret key with NEXT*PUBLIC* | Use server-only env vars in Server Actions               |
...
- [ ] **Environment vars**: No NEXT*PUBLIC* for secrets - audit .env files
...
| Exposed secrets              | HIGH          | Rotate keys immediately, remove NEXT*PUBLIC*, redeploy       |
...
_Pitfalls research for: Next.js 15 Landing Pages_
