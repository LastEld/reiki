...
<task type="auto">
  ...
  <done>
    Next.js 15 project exists at E:\Oleg with all default files. Build succeeds. Git ignores secrets and build artifacts.
  </done>
</task>
...
<task type="auto">
  ...
  <verify>
    ...
    3. `.env.example` exists and IS trackable by git
    4. Open `http://localhost:3000` after `npm run dev` -- page shows "Reiki Practice" h1 with Tailwind styling (large bold text, not unstyled)
  </verify>
  <done>
    metadataBase prevents build errors for relative metadata URLs. Image remotePatterns ready for external images. Environment variables properly scoped. Tailwind rendering confirmed with styled placeholder page. TypeScript strict mode enforced in builds.
  </done>
</task>
